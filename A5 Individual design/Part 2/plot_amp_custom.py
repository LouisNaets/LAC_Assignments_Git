...
def plot_amp_rpm(amp_df, mode_names, rpm, title=None):
    """Visualize modal amplitudes for given rotor speed.
    ...
    rpm : int, float
        Rotor speed to isolate and plot the modal amplitudes for.
    ...
    # filter by rotor speed !!! changed !!!
    df_rpm = amp_df.loc[(amp_df['Rotor speed'] == rpm)]
    nmodes = amp_df.loc['mode_number'].unique().size - 1
    assert nmodes == len(mode_names)

    # plot the data
    amp_modes_df = df_rpm.loc[:, df_rpm.columns.map(lambda s: 'phase' not in s)].iloc[:, 1:]
