...
IIIB.chord_max = IA.chord_max  # Maximum chord size [m] !! also shouldn't be scaled (due to transport reasons)!!
...
ax.legend()
fig.tight_layout()
fig.savefig('A5 Individual design/Figures 1/absolute_thickness.svg', format='svg')
fig.savefig('A5 Individual design/Figures 1/absolute_thickness.png', format='png')
...
def calculate_design_points(tc_data, margin, t_c_values):
    # Containers for results
    Cl_points = []
    Cd_points = []
    alpha_points = []

    # Process each airfoil dataset
    for airfoil_data in tc_data:
        # Extract columns for clarity
        alpha = airfoil_data[:, 0]  # Angle of attack
        cl = airfoil_data[:, 1]     # C_l
        cd = airfoil_data[:, 2]     # C_d

        # Find max(C_l)
        cl_max = np.max(cl)

        # Subtract margin
        cl_target = cl_max - margin

        # Ensure the target is within range
        if cl_target < np.min(cl) or cl_target > np.max(cl):
            raise ValueError("Target C_l is out of interpolation range.")

        # Find all points close to the target C_l
        indices = np.where(np.isclose(cl, cl_target, atol=1e-3))[0]

        if len(indices) > 0:
            # Multiple points, choose the one with the lowest C_d
            best_index = indices[np.argmin(cd[indices])]
            alpha_target = alpha[best_index]
            cd_target = cd[best_index]
        else:
            # Interpolate if no exact matches
            alpha_interp = interp1d(cl, alpha, kind='linear', bounds_error=False, fill_value="extrapolate")
            cd_interp = interp1d(cl, cd, kind='linear', bounds_error=False, fill_value="extrapolate")
            alpha_target = alpha_interp(cl_target).item()
            cd_target = cd_interp(cl_target).item()

        # Append results
        Cl_points.append(cl_target)
        Cd_points.append(cd_target)
        alpha_points.append(alpha_target)

    # Calculate Cl/Cd points
    Cl_Cd_points = np.array(Cl_points) / np.array(Cd_points)

    # Return all points along with t/c values
    return Cl_points, Cd_points, Cl_Cd_points.tolist(), alpha_points, t_c_values

# Example usage:
tc_data = [tc_241_data, tc_301_data, tc_360_data, tc_480_data]  # List of airfoil datasets
margin = 0.4  # Single margin for all airfoils
t_c_points = [24.1, 30.1, 36.0, 48.0]  # t/c values for each airfoil

Cl_points, Cd_points, Cl_Cd_points, alpha_points, t_c_points = calculate_design_points(tc_data, margin, t_c_points)

# Print results
#print("Cl_points:", Cl_points)
#print("Cd_points:", Cd_points)
#print("Cl/Cd points:", Cl_Cd_points)
#print("alpha_points:", alpha_points)
#print("t/c points:", t_c_points)

# Chosen design points for each airfoil (-0.5)
...
#(-0.4) !! 24.1 needs to manually be read, as it bugs out !!
Cl_points = [1.4139, 1.3327, 1.4775, 0.6425]
Cd_points = [0.014, 0.015845167853509663, 0.022305770046669497, 0.03226958614051973]
Cl_Cd_points = np.array(Cl_points)/np.array(Cd_points)
alpha_points = [9.2, 8.460834181078331, 6.745014849384812, 2.4959897337183183]
t_c_points = [24.1, 30.1, 36.0, 48.0]
...
for i, tc_data in enumerate(tc_datasets):
    ...
    plt.tight_layout()
    plt.savefig(f'A5 Individual design/Figures 1/{titles[i]}.svg', format='svg')
...
fig3.tight_layout()
fig4.tight_layout()
fig1.savefig('A5 Individual design/Figures 1/design_functions_clcd.png', format='png')
fig1.savefig('A5 Individual design/Figures 1/design_functions_clcd.svg', format='svg')
...
#Including minimum thickness because of airfoils available
...
plt.tight_layout()
plt.savefig('A5 Individual design/Figures 1/final_chord_twist_thickness.png', format='png')
plt.savefig('A5 Individual design/Figures 1/final_chord_twist_thickness.svg', format='svg')
#plt.show()
...
plt.savefig('A5 Individual design/Figures 1/3.1.png', format='png')

# Path for the file
inds_path = "./hawc_files/individual_design/res_hawc2s/individual_design_hawc2s_multitsr_u8003.ind"
# Load the data
inds_data = load_ind(inds_path)

# Path for the file
pwr_path = "./hawc_files/individual_design/res_hawc2s/individual_design_hawc2s_multitsr.pwr"
# Load the data
...
# Save the figure
fig7.tight_layout()
plt.savefig('A5 Individual design/Figures 1/3.3.svg', format='svg')

'''FIGURE 3.4'''

opt_path = './hawc_files/individual_design/data/individual_design_rigid.opt'
...
# Save the figure
fig8.tight_layout()
plt.savefig('A5 Individual design/Figures 1/3.4.svg', format='svg')
...
fig9.tight_layout()
plt.savefig('A5 Individual design/Figures 1/3.5.svg', format='svg')

'''
PART 4
'''

path_st_file_DTU10MW = "./hawc_files/individual_design/data/DTU_10MW_RWT_Blade_st.dat"
...
Blade_flex_st_path = './hawc_files/individual_design/data/individual_design_Blade_st.dat'
...
# Save the figure
fig10.tight_layout()
plt.savefig('A5 Individual design/Figures 1/4.1.png', format='png')
...
# Save the figure
fig11.tight_layout()
plt.savefig('A5 Individual design/Figures 1/4.2.svg', format='svg')
plt.savefig('A5 Individual design/Figures 1/4.2.png', format='png')
...
# Save the figure
fig12.tight_layout()
plt.savefig('A5 Individual design/Figures 1/4.3.svg', format='svg')
plt.savefig('A5 Individual design/Figures 1/4.3.png', format='png')
...
DTU10MW_flex_path = './hawc_files/individual_design/data/dtu_10mw_rigid.opt'
DTU10MW_flex_data = np.loadtxt(DTU10MW_flex_path, skiprows=1)

class DTU10MW_f_opt:
    V_o = DTU10MW_flex_data[:, 0]
    P = DTU10MW_flex_data[:, 3]
    T = DTU10MW_flex_data[:, 4]
...
axes12[0].plot(DTU10MW_f_opt.V_o, DTU10MW_f_opt.P, color = colors[0], label='DTU 10MW (rigid) rotor')
axes12[0].plot(f_opt.V_o, f_opt.P, color = colors[1], linestyle='--', label='New IIIB (rigid) design')
axes12[0].set_ylabel(r"Power [kW]")
axes12[0].set_xlabel(r"Wind speed [m/s]")
axes12[0].legend()
axes12[0].grid(True, linestyle = ':')

axes12[1].plot(DTU10MW_f_opt.V_o, DTU10MW_f_opt.T, color = colors[0], label='DTU 10MW (rigid) rotor')
axes12[1].plot(f_opt.V_o, f_opt.T, color = colors[1], linestyle='--', label='New IIIB (rigid) design')
axes12[1].set_ylabel(r"Thrust [kN]")
axes12[1].set_xlabel(r"Wind speed [m/s]")
axes12[1].legend()
axes12[1].grid(True, linestyle = ':')
...
# Save the figure
fig12.tight_layout()
plt.savefig('A5 Individual design/Figures 1/4.3_rigid.svg', format='svg')
plt.savefig('A5 Individual design/Figures 1/4.3_rigid.png', format='png')